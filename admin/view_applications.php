...
if (isset($_GET['delete_id'])) {
    ...
    $sqlDELETE = "DELETE FROM `create_application` WHERE `id`='$delete_id'";
...
<div class="container-fluid" style="margin-top:150px; max-width:90%;">
 <div class="row">
		      <div class="col-sm-12">
    ...
    <table class="table">
        <thead>
            <tr >
                <th scope="col" style="min-width:50px;">#</th>
                 <th scope="col" style="min-width:150px;">Student Name</th>
                <th scope="col" style="min-width:150px;">Universtiy Name</th>
                <th scope="col" style="min-width:150px;">Program Name</th>
        ...
        <tbody>
         ...
         $records_per_page = isset($_GET['items_per_page']) ? (int)$_GET['items_per_page'] : 10; // Default to 10 if not set
            ...
            $total_records_query = "SELECT COUNT(*) FROM `create_application`";
            $total_records_result = mysqli_query($conn, $total_records_query);
            $total_records_row = mysqli_fetch_array($total_records_result);
            $total_records = $total_records_row[0];

            // Fetch the records for the current page
            $sqllogistic = mysqli_query($conn, "SELECT * FROM `create_application` ORDER BY id DESC LIMIT $records_per_page OFFSET $offset");
            ...
            while($show=mysqli_fetch_assoc($sqllogistic)){
               ...
               $fetch_student_name=mysqli_fetch_assoc(mysqli_query($conn,"select * from student where id='".$show['student_name']."'"));
            $id++;
            ?>
            <tr>
                <td scope="row"><?php echo $id;?></td>
                <td scope="row"><?php echo $fetch_student_name['first_name']; ?></td>
                <td><?php echo $show['university_name'];?></td>
                   <td><?php echo $show['program_level'];?></td>

                <td>
                <div class="button-container">
                
                    <a href="#" onclick="confirmDelete(<?php echo $show['id']; ?>)" class="btn btn-primary Delete">Delete</a>
                    <a href="show_application.php?student=<?php echo urlencode($show['student_name']); ?>&programlevel=<?php echo urlencode($show['program_level']); ?>&universityname=<?php echo urlencode($show['university_name']); ?>" class="btn btn-primary" target="_blank">View Application</a>
