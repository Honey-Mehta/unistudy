...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student_id = $_POST['student6'] ?? null;
     print_r($student_id);
    ...
    if (isset($_FILES['highschool_diploma_file']) && !empty($_FILES['highschool_diploma_file']['name'])) {

        // Set file paths and sanitize the file name
        $highschool_diploma_file = basename($_FILES['highschool_diploma_file']['name']);

       
        $highschool_diploma_filePath = '../images/' . $highschool_diploma_file;
        
        // Check if the file is a valid PDF
        $fileType = mime_content_type($_FILES['highschool_diploma_file']['tmp_name']);
        ...
        if (move_uploaded_file($_FILES['highschool_diploma_file']['tmp_name'], $highschool_diploma_filePath)) {

            // Prepare SQL query to update student record
            $sql = "UPDATE `student` SET `highschool_diploma` = '$highschool_diploma_file' WHERE `id` = '$student_id'";
