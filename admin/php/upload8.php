...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student_id = $_POST['student8'] ?? null;
     print_r($student_id);
    ...
    if (isset($_FILES['resume_file']) && !empty($_FILES['resume_file']['name'])) {
        ...
        $resume_filePath = '../images/' . $resume_file;
        
        // Check if the file is a valid PDF
        $fileType = mime_content_type($_FILES['resume_file']['tmp_name']);
        ...
        if (move_uploaded_file($_FILES['resume_file']['tmp_name'], $resume_filePath)) {
            ...
            $sql = "UPDATE `student` SET `resume` = '$resume_file' WHERE `id` = '$student_id'";
