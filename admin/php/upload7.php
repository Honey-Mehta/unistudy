...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student_id = $_POST['student7'] ?? null;
     print_r($student_id);
    ...
    if (isset($_FILES['medium_of_instruction_file']) && !empty($_FILES['medium_of_instruction_file']['name'])) {

        // Set file paths and sanitize the file name
        $medium_of_instruction_file = basename($_FILES['medium_of_instruction_file']['name']);

       
        $medium_of_instruction_filePath = '../images/' . $medium_of_instruction_file;
        
        // Check if the file is a valid PDF
        $fileType = mime_content_type($_FILES['medium_of_instruction_file']['tmp_name']);
        ...
        if (move_uploaded_file($_FILES['medium_of_instruction_file']['tmp_name'], $medium_of_instruction_filePath)) {

            // Prepare SQL query to update student record
            $sql = "UPDATE `student` SET `medium_of_instruction` = '$medium_of_instruction_file' WHERE `id` = '$student_id'";
