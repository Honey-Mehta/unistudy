...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student_id = $_POST['student4'] ?? null;
     print_r($student_id);
    ...
    if (isset($_FILES['college_diploma_file']) && !empty($_FILES['college_diploma_file']['name'])) {

        // Set file paths and sanitize the file name
        $college_diploma_file = basename($_FILES['college_diploma_file']['name']);

       
        $college_diploma_filePath = '../images/' . $college_diploma_file;
        
        // Check if the file is a valid PDF
        $fileType = mime_content_type($_FILES['college_diploma_file']['tmp_name']);
        ...
        if (move_uploaded_file($_FILES['college_diploma_file']['tmp_name'], $college_diploma_filePath)) {

            // Prepare SQL query to update student record
            $sql = "UPDATE `student` SET `college_diploma` = '$college_diploma_file' WHERE `id` = '$student_id'";
