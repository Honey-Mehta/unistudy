...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student_id = $_POST['student3'] ?? null;
     print_r($student_id);
    ...
    if (isset($_FILES['college_transcript_file']) && !empty($_FILES['college_transcript_file']['name'])) {

        // Set file paths and sanitize the file name
        $college_transcript_file = basename($_FILES['college_transcript_file']['name']);

       
        $college_transcript_filePath = '../images/' . $college_transcript_file;
        
        // Check if the file is a valid PDF
        $fileType = mime_content_type($_FILES['college_transcript_file']['tmp_name']);
        ...
        if (move_uploaded_file($_FILES['college_transcript_file']['tmp_name'], $college_transcript_filePath)) {

            // Prepare SQL query to update student record
            $sql = "UPDATE `student` SET `college_transcript` = '$college_transcript_file' WHERE `id` = '$student_id'";
